...
type Order = {
  ...
  saProcessedBy?: { fullName: string };
  saProcessedAt?: string;
};

export default function SASection({ permission }: { permission: Permission }) {
  ...
  const handleMarkDone = async (orderId: string) => {
    ...
    await fetch('/api/orders', {
      ...
      body: JSON.stringify({ orderId, status: 'SB_PENDING', section: 'sa' })
    });
    fetchOrders();
  };
  ...
  const handleMarkFailed = async (orderId: string) => {
    ...
    await fetch('/api/orders', {
      ...
      body: JSON.stringify({ orderId, status: 'SA_FAILED', section: 'sa' })
    });
    fetchOrders();
  };
  ...
  const filteredOrders = orders.filter(order => {
    if (filter === 'pending') return order.status === 'SA_PENDING';
    if (filter === 'done') return order.status === 'SA_DONE' || order.status === 'SB_PENDING' || order.status.includes('SB_') || order.status.includes('SC_') || order.status.includes('PACKAGING') || order.status.includes('DISPATCH');
    if (filter === 'failed') return order.status === 'SA_FAILED';
    return order.status.includes('SA_') || order.status.includes('SB_') || order.status.includes('SC_') || order.status.includes('PACKAGING') || order.status.includes('DISPATCH');
  });
    ...
    return (
      <div className="flex items-center justify-center py-16 md:py-32 px-4">
        <div className="text-center">
          ...
          <p className="text-base md:text-xl text-gray-600 font-medium">Loading Section A orders...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4 md:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-gray-900">⚙️ Section A Processing</h2>
          <p className="text-sm md:text-base text-gray-600 mt-1 md:mt-2">Process accepted orders in Section A</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
        <div className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-lg border-l-4 border-orange-500">
          <div className="text-2xl md:text-3xl font-bold text-orange-600">
            {orders.filter(o => o.status === 'SA_PENDING').length}
          </div>
        ...
        <div className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-lg border-l-4 border-green-500">
          <div className="text-2xl md:text-3xl font-bold text-green-600">
            {orders.filter(o => o.status === 'SA_DONE' || o.status.includes('SB_') || o.status.includes('SC_') || o.status.includes('PACKAGING') || o.status.includes('DISPATCH')).length}
          </div>
        ...
        <div className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-lg border-l-4 border-red-500">
          <div className="text-2xl md:text-3xl font-bold text-red-600">
            {orders.filter(o => o.status === 'SA_FAILED').length}
          </div>
        ...
        <div className="bg-white rounded-xl md:rounded-2xl p-4 md:p-6 shadow-lg border-l-4 border-blue-500 col-span-1 lg:col-span-1">
          <div className="text-2xl md:text-3xl font-bold text-blue-600">
            {orders.filter(o => o.status.includes('SA_') || o.status.includes('SB_') || o.status.includes('SC_') || o.status.includes('PACKAGING') || o.status.includes('DISPATCH')).length}
          </div>
          <div className="text-xs md:text-base text-gray-600 font-medium">Total SA Orders</div>
        </div>
      </div>
        ...
        {filteredOrders.map(order => (
          <motion.div
            ...
            className={`bg-white rounded-xl md:rounded-2xl shadow-xl p-4 md:p-6 border-2 ${
              order.status === 'SA_PENDING' ? 'border-orange-300 bg-orange-50/30' :
              order.status === 'SA_FAILED' ? 'border-red-300 bg-red-50/30' :
          ...
          >
            ...
            <div className="flex items-start justify-between gap-2 mb-3 md:mb-4 overflow-x-auto pb-1">
              ...
              <div className="flex flex-col sm:flex-row gap-2 flex-shrink-0">
                <span className={`px-2 md:px-4 py-1 md:py-2 rounded-lg md:rounded-xl font-bold text-[10px] md:text-sm whitespace-nowrap ${
                  order.status === 'SA_PENDING' ? 'bg-orange-500 text-white' :
                  order.status === 'SA_FAILED' ? 'bg-red-500 text-white' :
              ...
              {order.items.map((item, idx) => (
                <div key={idx} className="flex flex-col sm:flex-row sm:justify-between gap-2 bg-white rounded-lg p-3 border border-gray-200">
                  <div className="flex-1">
                    <span className="font-semibold text-sm md:text-base text-orange-600">Type {item.itemType}</span>
            ...
            {permission === 'read_write' && order.status === 'SA_PENDING' && (
